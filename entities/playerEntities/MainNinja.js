class MainNinja {
  ...
  loadAnimations() {
    this.animations['idlehud'] = new Animator(
      this.spritesheet,
      3800 + 30,
      1500,
      135,
  ...
  update() {
    ...
    const MAX_WALK = 93.75 * 3 * this.baseSpeedMultiplier;
    const MAX_RUN = 153.75 * 3 * this.baseSpeedMultiplier;

    const DEC_REL = 182.8125 * this.baseSpeedMultiplier;

    const MAX_FALL = 270 * 3 * this.baseSpeedMultiplier;
    ...
    if (
      ...
      if (this.action.substring(0, 6) != 'attack' && this.action != 'throw') {
      ...
      if (this.game.jump && this.action.substring(0, 6) != 'attack') {
        // jump key press
    ...
    this.game.entities.forEach(function (entity) {
      if (entity.BB && self.BB.collide(entity.BB)) {
        ...
        if (entity instanceof Ground) {
          if (self.lastBB.top >= entity.BB.bottom) {
            // top collision
  ...
  throwMech() {
    if (
      ...
      (this.game.throw || this.action === 'throw')
    ) {
  ...
  attackMech() {
    if (
      this.action != 'throw' &&
      (this.game.attack || this.action.substring(0, 6) === 'attack')
    ) {
  ...
  potionMech() {
    if (this.game.camera.healthPotion.cdTimer >= 0) {
      // display cool down
      this.game.camera.healthPotion.cdTimer -= this.game.clockTick;
      //console.log(this.game.camera.potion.cdTimer);
    } else if (
      this.game.healthPotion &&
      this.game.camera.healthPotion.quantity > 0
    ) {
      this.hp += 100;
      //show heal amout
      let healIndicator = new dmgIndicator(
        this.game,
        this.BB.x,
        this.BB.y,
        100,
      ...
      this.game.camera.healthPotion.quantity -= 1;
      this.game.camera.healthPotion.cdTimer = 10;
    }

    if (this.game.camera.speedPotion.cdTimer >= 0) {
      this.game.camera.speedPotion.cdTimer -= this.game.clockTick;
    } else if (
      this.game.speedPotion &&
      this.game.camera.speedPotion.quantity > 0
    ) {
      this.game.camera.speedPotion.quantity -= 1;
      this.game.camera.speedPotion.cdTimer = 30;
      this.attackDmg *= 1.5;
      setTimeout(() => {
        this.attackDmg /= 1;
      }, 30e3);
  ...
  drawCharacterHud(ctx) {
    ...
    this.animations['idlehud'].drawFrame(
      this.game.clockTick,
      ctx,
      15,
